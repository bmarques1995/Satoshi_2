...
{
    VKImGUIData vkImGUIData;
    vkImGUIData.CommandBuffer = m_CommandBuffer;
    vkImGUIData.RenderPass = m_RenderPass;
    vkImGUIData.CommandPool = m_CommandPool;
    vkImGUIData.VulkanInfo.Instance = m_Instance;
    vkImGUIData.VulkanInfo.Device = m_LogicalDevice;
    vkImGUIData.VulkanInfo.PhysicalDevice = m_CurrentDevice;
    vkImGUIData.VulkanInfo.Queue = m_GraphicsQueue;
    vkImGUIData.VulkanInfo.QueueFamily = m_GraphicsQueueFamilyIndex;
    vkImGUIData.VulkanInfo.CheckVkResultFn = [](VkResult err)
    {
        if (err == VK_SUCCESS)
            return;
        Console::CoreError("[vulkan] Error: VkResult = {0}\n", err);
        assert(err < 0);
    };
    vkImGUIData.VulkanInfo.Subpass = 0;
    vkImGUIData.VulkanInfo.PipelineCache = VK_NULL_HANDLE;
    vkImGUIData.VulkanInfo.PipelineCache = VK_NULL_HANDLE;
    vkImGUIData.VulkanInfo.DescriptorPool = m_DescriptorPool;
    vkImGUIData.VulkanInfo.MinImageCount = m_MinImageCount;
    vkImGUIData.VulkanInfo.ImageCount = m_ImageCount;
    vkImGUIData.VulkanInfo.MSAASamples = VK_SAMPLE_COUNT_1_BIT;
    vkImGUIData.VulkanInfo.Allocator = nullptr;
	return vkImGUIData;
}
...
void Satoshi::VKContext::CreateSurface()
{
    ...
    createInfo.hwnd = std::any_cast<HWND>(Application::GetInstance()->GetWindow()->GetNativeWindow());
...
void Satoshi::VKContext::CreateDescriptorPool()
{
    ...
    VkResult vk_r = vkCreateDescriptorPool(m_LogicalDevice, &pool_info, nullptr, &m_DescriptorPool);
    assert(vk_r == VK_SUCCESS);
}
