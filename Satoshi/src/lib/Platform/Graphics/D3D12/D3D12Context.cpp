	...
	m_BackBuffersAmount(numBackBuffers), m_FramesInFlightAmount(numFramesInFlight)
{
	...
	CreateCommandList();
	CreateFence();
	CreateSwapChain(std::any_cast<HWND>(Application::GetInstance()->GetWindow()->GetNativeWindow()));
...
std::any Satoshi::D3D12Context::GetImGUIData()
{
	...
	d3d12Data.Device = m_Device.Get();
	d3d12Data.RTVFormat = m_RTVFormat;
	d3d12Data.DescriptorHeap = m_SourceDescHeap.Get();
	return d3d12Data;
}
